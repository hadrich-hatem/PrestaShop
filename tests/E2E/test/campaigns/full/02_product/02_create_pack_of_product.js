...
scenario('Create pack product in the Back Office', client => {
  ...
  scenario('Fill "Basic settings" form', client => {
      ...
      return promise
        ...
        .then(() => {
          if(global.isVisible) {
            ...
            expect(global.isVisible, "BOOM-5787: The product is exist in the result of search").to.be.false;
          } else {
            client.pause(1000);
          }
        });
    ...
    test('should set the "Quantity" of product', () => client.waitAndSetValue(AddProductPage.quantity_shortcut_input, data.common.quantity, 2000));
    ...
    commonScenarios.clickOnCoverAndSave(client);
    test('should set the "Legend picture"', () => client.waitAndSetValue(AddProductPage.picture_legend_textarea_en, data.pack.name, 3000));
    ...
    test('should click on "Yes" modal button', () => client.waitForVisibleAndClick(AddProductPage.delete_confirmation_button.replace('%BUTTON', 'Yes')));
    commonScenarios.addProductFeature(client, "Frame Size", 1, "Elastane");
    commonScenarios.addProductFeature(client, "Frame Size", 2, "Polyester");
    commonScenarios.addProductFeature(client, "Frame Size", 3, "Wool");
    ...
    test('should check that the third value of "Frame Size" feature does exist', () => client.isExisting(productPage.product_features.replace('%F', 'Polyester')));
    test('should check that the forth value of "Frame Size" feature does exist', () => client.isExisting(productPage.product_features.replace('%F', 'Wool')));
    test('should go back to the Back Office', () => client.switchWindow(0));
  ...
  scenario('Fill "Pricing" form', client => {
    ...
    test('should click on the "Sign in" link', () => client.waitForExistAndClick(AccessPageFO.sign_in_button, 3000));
  ...
  scenario('Fill the product options form', client => {
    ...
    test('should check that the success alert message is well displayed', () => client.waitForExistAndClick(AddProductPage.close_validation_button));
    test('should go to the Front Office', () => client.switchWindow(1));
    test('should check that the product online only flag does exist', () => client.isExisting(productPage.product_online_only_flag, 20000));
  ...
  scenario('Check the products quantity', client => {
    ...
    test('should check that the pack product quantity is well decremented', () => {
      return promise
        .then(() => client.checkTextValue(Stock.product_quantity.replace('%O', 1).replace('%I', 7), '898', 'equal', 5000))
        .then(() => client.refresh());
    });
    test('should search for the product by name and dimension', () => {
      return promise
    ...
    test('should check that the pack product quantity is well decremented', () => {
      return promise
        .then(() => client.checkTextValue(Stock.product_quantity.replace('%O', 1).replace('%I', 7), '294', 'equal', 5000))
        .then(() => client.refresh());
    });
  }, 'common_client');
  scenario('Change the default pack stock management', client => {
  ...
  scenario('Check the products quantity', client => {
    ...
    test('should check that the pack product quantity is well decremented', () => {
      return promise
        .then(() => client.checkTextValue(Stock.product_quantity.replace('%O', 1).replace('%I', 7), '896', 'equal', 5000))
        .then(() => client.refresh());
    });
    test('should search for the product by name and dimension', () => {
      return promise
    ...
    test('should check that the pack product quantity is well decremented', () => {
      return promise
        .then(() => client.checkTextValue(Stock.product_quantity.replace('%O', 1).replace('%I', 7), '288', 'equal', 5000))
        .then(() => client.refresh());
    });
  }, 'common_client');
