...
scenario('Create virtual Product in the Back Office', client => {
  ...
  scenario('Fill "Basic settings" form', client => {
    ...
    commonScenarios.clickOnCoverAndSave(client);
    test('should set the "Legend picture"', () => client.waitAndSetValue(AddProductPage.picture_legend_textarea_en, data.virtual.name, 2000));
    ...
    /** @TODO: check that the custome value is well disabled when we choose a predefined value
     ...
     */
    test('should click on "SAVE"', () => client.waitForExistAndClick(AddProductPage.save_product_button, 3000));
    commonScenarios.addProductFeature(client, "Compositions", 1, '', "Azerty", "custom_value");
    /** @TODO: check that the element predefined value is well disabled when the custom value is not empty
     ...
     */
    test('should click on "Delete" icon of the second feature', () => client.waitForExistAndClick(AddProductPage.delete_feature_icon.replace('%POS', 2), 2000));
    test('should click on "Yes" modal button', () => client.waitForVisibleAndClick(AddProductPage.delete_confirmation_button.replace('%BUTTON', 'Yes')));
  ...
  scenario('Fill "Quantities" form', client => {
    ...
    scenario('Check the "Availability date"', client => {
      test('should set the "Availability date"', () => client.waitAndSetValue(AddProductPage.availability_date_input, common.getCustomDate(30), 5000));
      test('should click on "Save" button', () => client.waitForExistAndClick(AddProductPage.save_product_button, 3000));
      test('should check that the success alert message is well displayed', () => client.waitForExistAndClick(AddProductPage.close_validation_button));
      test('should go to the Front Office', () => client.switchWindow(1));
      test('should click on "Product Details" tab', () => client.scrollWaitForExistAndClick(productPage.product_tab_list.replace('%I', 2), 0, 5000));
      test('should check that the "Availability date" is equal to "' + common.getCustomDate(30) + '"', () => {
        return promise
          .then(() => client.scrollTo(productPage.product_availability_date, 0, 5000))
  ...
  scenario('Fill the product options form', client => {
    ...
    test('should check that the product is well displayed', () => client.isExisting(productPage.productLink.replace('%PRODUCTNAME', 'VRT' + date_time)));
    test('should go to the "Home" page', () => client.waitForExistAndClick(AccessPageFO.logo_home_page));
    test('should search for the product', () => client.searchByValue(SearchProductPage.search_input, SearchProductPage.search_button, data.virtual.name + date_time));
    ...
    test('should go back to the Back Office', () => client.switchWindow(0));
    test('should click on "Display condition on product page" checkbox', () => client.waitForExistAndClick(AddProductPage.options_show_condition_checkbox, 3000));
